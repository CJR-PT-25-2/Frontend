...
export default function ReviewDetailPage() {
  const { comentarioId } = useParams();
  const router = useRouter();
  const { user } = useAuth();

  const [loading, setLoading] = useState(true);
  const [review, setReview] = useState<AvaliacaoProduto | null>(null);
  ...
  const fetchReview = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/avaliacao-produto/${comentarioId}`);
...
const getAvatarUrl = (usuario: Usuario | undefined | null) => {
  ...
  const handleSendComment = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/comentarios`, {
        ...
        body: JSON.stringify({
          conteudo: newComment,
          avaliacao_loja_id: null,
          avaliacao_produto_id: Number(review.id),
  ...
  const saveReviewEdit = async () => {
    ...
    const token = getToken();
    try {
      const res = await fetch(`${API_URL}/avaliacao-produto/${review.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
            ...
            {hasComments ? (
              <div className="border-l border-gray-400/40 pl-8 space-y-9">
                {review.Comentarios?.map((resp) => {
                  ...
                  // CÓDIGO CORRIGIDO
                  const isStoreOwner =
                    Number(resp.usuario_id) === Number(review.Produto.Loja.donoId);
