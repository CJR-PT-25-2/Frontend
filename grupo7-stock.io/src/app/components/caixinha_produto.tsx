    ...
    export default function Caixa_prod({ 
        ...
        const handleNavegationprod = () =>{
            router.push(`produto/${id}`);
        }


        return (
            <div className=" bg-white w-50 h-75 rounded-2xl shadow-lg p-4 flex flex-col flex-shrink-0  ">
                <div className="relative items-center justify-center ">
                    { finalImageUrl ? (
                    <img src={`http://localhost:3001${finalImageUrl}`} alt={'Imagem do produto '} className=" w-40 h-40 object-contain mb-4"/>
                    ) : (
                    ...
                    { finalLojaurl &&
                    <div className="rounded-full overflow-hidden w-15 h-15 absolute top-0 right-0 z-10 cursou-pointer">
                        <img src={`http://localhost:3001${finalLojaurl}`} alt="logo" className="w-15 h-15 absolute
                                                                        top-0 right-0
                                                                        z-10"/>  
                    </div>
                ...
                <div className="text-black  font-semibold text-lg mb-2 items-start cursor-pointer hover:text-[#d6993c]" onClick={handleNavegationprod}>
                    {nome}
            </div>
